"""
infra/journal.py — Pinescript Bot
...
class Journal:
    ...
    def log_order_event(self, **kwargs) -> None:
        """Write an order/audit event to Google Sheets without risking the DB journal."""
        try:
            self._gsheet.log_order_event(**kwargs)
        except Exception as e:
            logger.error(f"GSheet order event sync failed: {e}")
