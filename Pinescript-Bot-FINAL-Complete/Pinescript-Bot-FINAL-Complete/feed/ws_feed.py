"""
feed/ws_feed.py  —  Pinescript Bot  (DELTA-TICK-FIX-v1)
"""
...
_WS_LIVE    = "wss://public-socket.india.delta.exchange"
...
class CandleFeed:
    ...
    async def _process_ws_candle(self, data: dict) -> None:
        ...
        try:
            o = float(data.get("open",   data.get("o", 0)))
            h = float(data.get("high",   data.get("h", 0)))
            l = float(data.get("low",    data.get("l", 0)))
            c = float(data.get("close",  data.get("c", 0)))
            v = float(data.get("volume", data.get("v", 0)))
        except (TypeError, ValueError):
            return
        ...
        if current_boundary > self._last_candle_boundary:
            if not self._df.empty:
                feed_name = "Binance" if BINANCE_SIGNAL_FEED else "Delta"
                try:
                ...
                # Volume must come from the same OHLCV feed as the TradingView
                # reference chart. Never mix Binance prices with Delta volume.
                _idx = self._df.index[-1]
                logger.info(
                    f"[FEED] Confirmed volume={float(self._df.at[_idx, 'volume']):.0f} "
                    f"from {feed_name} (same-source Pine parity)"
                )
