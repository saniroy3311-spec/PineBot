"""
feed/fills_feed.py — Pinescript Bot  |  FIX-FILLS-WS
════════════════════════════════════════════════════════════════════════════════
...
"v2/user_trades" channel. Delta pushes a fill event within milliseconds of any order
...
──────────────
  ...
  Auth:    send {"type": "key-auth", "payload": {"api-key": ..., "signature": ...,
...
class FillsFeed:
    ...
    async def _connect_and_listen(self) -> None:
        ...
        async with websockets.connect(
            ...
            async for raw in ws:
                ...
                # Current low-latency channel uses type="v2/user_trades" and
                # compact field names; verbose user_trades remains accepted.
    ...
    async def _handle_fill(self, msg: dict) -> None:
        """
        Process a fill event from the v2/user_trades channel.
        ...
        # Symbol check — v2/user_trades uses compact key `sy`.
        fill_symbol = (data.get("symbol") or data.get("sy") or "").upper()
        if fill_symbol != self._ws_symbol:
            return

        # v2/user_trades intentionally omits order_type. The emergency bracket is
        # the only exchange-side close order while manager._bracket_active=True.
        # Python-owned exits cancel that bracket *before* placing their reduce-only
        # market close, so a close-side fill while the flag is still true is the
        # bracket (or an equivalent external/manual close that must be reconciled).
        order_type = (data.get("order_type") or data.get("stop_order_type") or "").lower().strip()
        bracket_active = bool(getattr(self._order_mgr, "_bracket_active", False))
        pos_is_long = getattr(self._order_mgr, "_is_long", None)
        side = str(data.get("side") or data.get("S") or "").lower()
        expected_close_side = "sell" if pos_is_long is True else ("buy" if pos_is_long is False else "")
        explicit_bracket = order_type in _BRACKET_ORDER_TYPES
        inferred_bracket = bracket_active and expected_close_side and side == expected_close_side
        if not (explicit_bracket or inferred_bracket):
            logger.debug(
                f"[FILLS] Non-exit fill ignored: order_type={order_type!r} "
                f"side={side!r} bracket_active={bracket_active}"
            )
            return

        # Extract fill price — compact v2 key is `p`.
        fill_price_raw = (
            data.get("fill_price") or data.get("price") or data.get("p") or
            data.get("average") or 0.0
        )
        try:
        ...
        # Emergency bracket contains only a stop-loss leg in this repo.
        if order_type in _TP_ORDER_TYPES:
            reason = "Bracket TP (exchange-side)"
        else:
            reason = "Emergency Bracket SL (exchange-side)"
