"""
indicators/engine.py — Pinescript Bot (numba-accelerated, Pine-exact)
...
def _ema_njit(arr: np.ndarray, length: int) -> np.ndarray:
    ...
    TradingView's EMA recurrence uses alpha=2/(length+1) and initializes the
    recursive state from the first non-na source value.  This matters for exact
    warm-up parity, especially because this strategy applies an EMA(5) to ADX.
    """
    n = len(arr)
    out = np.full(n, np.nan)
...
def _dmi_series(
    ...
    # TradingView's published DMI construction keeps the first directional
    # movement value as na (ta.change() has no prior bar), then applies RMA.
    valid_dm = up_move.notna() & down_move.notna()
    plus_dm = pd.Series(np.nan, index=high.index, dtype=float)
    minus_dm = pd.Series(np.nan, index=high.index, dtype=float)
    plus_dm.loc[valid_dm] = np.where(
        (up_move.loc[valid_dm] > down_move.loc[valid_dm]) & (up_move.loc[valid_dm] > 0),
        up_move.loc[valid_dm], 0.0,
    )
    minus_dm.loc[valid_dm] = np.where(
        (down_move.loc[valid_dm] > up_move.loc[valid_dm]) & (down_move.loc[valid_dm] > 0),
        down_move.loc[valid_dm], 0.0,
    )

    tr       = _true_range(high, low, close)
    atr_di   = _rma(tr, di_len).replace(0.0, np.nan)

    plus_di  = 100.0 * _rma(plus_dm,  di_len) / atr_di
    minus_di = 100.0 * _rma(minus_dm, di_len) / atr_di

    # Pine's DMI uses denominator 1 when +DI + -DI is zero. Keep pre-warmup
    # values as na so ADX does not seed before the DI series is valid.
    sum_di = plus_di + minus_di
    denom = sum_di.where(sum_di != 0.0, 1.0)
...
def compute_full_series(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ema_trend"] = _ema(close, EMA_TREND_LEN).values
    out["ema_fast"]  = _ema(close, EMA_FAST_LEN).values
    # compatibility aliases for old CSV tooling
    out["ema200"] = out["ema_trend"]
    out["ema50"] = out["ema_fast"]
